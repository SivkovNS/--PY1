...
def to_csv_file(filename, headers, rows, delimiter, new_line):
    with open(filename, 'w') as file:
        titles = [str(headers[i]) for i in range(len(headers))]
        titles = delimiter.join(titles)
        file.write(titles)
        file.write(new_line)
        for i in range(len(rows)):
            list_elem = rows[i] # взяли список из списка списков
            gro2 = [str(list_elem[b]) for b in range(len(list_elem))]
            gro2 = delimiter.join(gro2)
            file.write(gro2)
            file.write(new_line)


to_csv_file(OUTPUT_FILE, headers_list, data, sign_, separator_)
